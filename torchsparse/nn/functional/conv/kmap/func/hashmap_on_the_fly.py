...
def _can_use_int32_hashmap(spatial_range: Optional[Tuple[int]]) -> bool:
    if spatial_range is None or torchsparse.tensor.get_allow_negative_coordinates():
        return False
...
def build_kmap_implicit_GEMM_hashmap_on_the_fly(
    ...
    use_compact_subm = (
        ...
        and hasattr(torchsparse.backend, "build_kernel_map_subm_hashmap_compact")
        and hasattr(torchsparse.backend, "build_kernel_map_subm_hashmap_compact_int32")
    )
    if subm:
        if use_compact_subm:
            func = (
                torchsparse.backend.build_kernel_map_subm_hashmap_compact_int32
                if use_int32_hashmap
                else torchsparse.backend.build_kernel_map_subm_hashmap_compact
            )
        else:
            func = (
                torchsparse.backend.build_kernel_map_subm_hashmap_int32
                if use_int32_hashmap
                else torchsparse.backend.build_kernel_map_subm_hashmap
            )
    else:
        func = (
            torchsparse.backend.build_kernel_map_downsample_hashmap_int32
            if use_int32_hashmap
            else torchsparse.backend.build_kernel_map_downsample_hashmap
        )
    to_insert = False
    ...
    hashmap_cls = (
        torchsparse.backend.GPUHashTable32
        if kmap["hashmap_keys"].dtype == torch.int32
        else torchsparse.backend.GPUHashTable
    )
...
def build_kmap_Gather_Scatter_hashmap_on_the_fly(
    ...
    if (
        ...
        and hasattr(torchsparse.backend, "compact_out_in_map_ordered")
    ):
        nbmaps, nbsizes, _, _ = torchsparse.backend.compact_out_in_map_ordered(
            kmap["out_in_map"]
        )
    else:
...
def build_kmap_Fetch_on_Demand_hashmap_on_the_fly(
    ...
    if (
        ...
        and hasattr(torchsparse.backend, "compact_out_in_map_fod")
    ):
