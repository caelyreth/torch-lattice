...
class TargetBranchModel(nn.Module):
    def __init__(self, pre: nn.Module, target_conv: spnn.Conv3d) -> None:
        super().__init__()
        self.pre = pre
        self.target_conv = target_conv

    def forward(self, x: SparseTensor, target: SparseTensor) -> SparseTensor:
        return self.target_conv(self.pre(x), coordinates=target)
...
@contextmanager
def _conv_dataflow(
    dataflow: Dataflow, *, kmap_mode: str | None = None
) -> Iterator[None]:
...
def _build_case(
    ...
    *,
    ...
    if family == "point_voxel":
        ...
        case = _generative_transpose_case(
            case_dir,
            name=name,
            seed=seed,
            rng=rng,
        ...
        case = _classifier_case(
            ...
            train_steps=train_steps,
            device=device,
            quantize_bits=rng.choice((4, 8)),
        )
    elif family == "sparse_feature_chain":
        case = _sparse_feature_chain_case(
            case_dir,
            name=name,
            seed=seed,
            rng=rng,
...
def _classifier_case(
    ...
    *,
    ...
    model = SparseChainModel(
        stages, global_pool=spnn.GlobalAvgPool(), head=nn.Linear(hidden, out_channels)
    )
    target = (
        torch.randn(
            (spec.batch_size, out_channels), generator=_torch_generator(seed + 17)
        )
        * 0.25
    )
...
def _transpose_case(
    ...
    *,
    ...
    model = SparseChainModel(
        [
            spnn.Conv3d(
                in_channels, hidden, kernel_size=(2, 1, 1), stride=(2, 1, 1), bias=False
            ),
            spnn.ConvTranspose3d(
                hidden, out_channels, kernel_size=(2, 1, 1), stride=(2, 1, 1), bias=True
            ),
        ]
    )
    ...
    return FuzzCase(
        ...
        3e-3,
        {
            "input": _spec_metadata(spec),
            "train_steps": train_steps,
            "ops": ["conv_stride2", "conv_transpose_stride2"],
        },
    )
    ...
    *,
    ...
    return FuzzCase(
        ...
        3e-3,
        {
            "input": _spec_metadata(spec),
            "train_steps": 0,
            "reference": "python_generative_transpose",
            "ops": ["generative_conv_transpose_stride2"],
        },
    )
...
def _point_voxel_case(
    ...
    *,
    ...
    points = torch.rand(
        (point_count, 3), generator=_torch_generator(seed + 100)
    ) * rng.choice((2.0, 3.0, 4.0))
    features = (
        torch.randn((point_count, channels), generator=_torch_generator(seed + 101))
        * 0.5
    )
    ...
    features_value = builder.dense_argument(
        "features", f"tensor<?x{channels}xf32>", channels=channels
    )
    ...
    lower_fx_artifact(
        builder, model, inputs=(points_value, features_value, batch_value, active_value)
    )
...
def _random_sparse_stages(
    ...
    *,
    ...
    for index in range(depth):
        next_channels = (
            out_channels if index == depth - 1 else rng.choice((3, 4, 5, 6, 8))
        )
        conv_kind = rng.choice(("conv1", "subm1", "subm3", "subm3_d2"))
        if conv_kind == "conv1":
            stages.append(
                spnn.Conv3d(
                    channels,
        ...
        elif conv_kind in {"subm3", "subm3_d2"}:
            stages.append(
                spnn.SubmConv3d(
                    channels,
                    next_channels,
                    kernel_size=3,
                    dilation=2 if conv_kind == "subm3_d2" else 1,
                    bias=rng.choice((True, False)),
                )
            )
        else:
...
def _conv_block(
    in_channels: int, out_channels: int, rng: random.Random, *, prefix: str
) -> nn.Sequential:
    layers: list[nn.Module] = [
        spnn.Conv3d(
            in_channels, out_channels, kernel_size=1, bias=rng.choice((True, False))
        ),
        _activation(rng),
    ]
    if rng.random() < 0.4:
...
def _activation(rng: random.Random) -> nn.Module:
    choice = rng.choice(
        ("relu", "leaky_relu", "silu", "gelu", "sigmoid", "tanh", "softplus")
    )
    if choice == "relu":
        return spnn.ReLU()
...
def _random_sparse_input(spec: SparseInputSpec, rng: random.Random) -> SparseTensor:
    ...
    feats = (
        torch.randn(
            (len(coords), spec.channels),
            generator=_torch_generator(rng.randrange(1 << 31)),
            dtype=spec.dtype,
        )
        * 0.5
    )
...
def _train_sparse(
    model: nn.Module, *inputs: SparseTensor, steps: int, device: torch.device
) -> None:
    if steps <= 0:
        return
    model.train()
...
def _dense_output(
    model: nn.Module, x: SparseTensor, device: torch.device
) -> torch.Tensor:
...
def _sparse_output(
    model: nn.Module, *inputs: SparseTensor, device: torch.device
) -> SparseTensor:
    model.to(device).eval()
    with torch.no_grad():
...
def _generative_transpose_reference(
    model: SparseChainModel, tensor: SparseTensor
) -> SparseTensor:
    conv = model.stages[0]
...
def _save_sparse_inputs(
    ...
    *,
    ...
    values = {
        f"{name}_coords": tensor.coords.cpu(),
        f"{name}_features": tensor.feats.cpu(),
