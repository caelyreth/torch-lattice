...
def _parse_args() -> argparse.Namespace:
    ...
    run.add_argument(
        "--package", choices=("torch_lattice", "torchsparse"), required=True
    )
...
def _subprocess_run(
    package: PackageName, output: Path, *, cases: int, seed: int, device: str
) -> None:
    command = [
        sys.executable,
...
def _run_package(
    ...
    with _gather_scatter_conv(package):
        for index, case in enumerate(_cases(cases, seed)):
            ...
            model = (
                _model(package, spnn, lattice, case.family).to(selected_device).eval()
            )
            with torch.no_grad():
                y = model(x)
...
def _model(package: PackageName, spnn, lattice, family: Family):
    ...
    if family == "spatial_subm_mapping":
        ...
        return torch.nn.Sequential(
            spnn.Conv3d(4, 3, kernel_size=3, stride=1, bias=True)
        )
    if family == "stride2_forward":
        return torch.nn.Sequential(
            spnn.Conv3d(4, 3, kernel_size=(2, 1, 1), stride=(2, 1, 1), bias=True)
        )
