...
def _device_params(config: pytest.Config) -> list[object]:
    ...
    for name in names:
        if name == 'cuda' and not torch.cuda.is_available():
            params.append(pytest.param(name, id=name, marks=pytest.mark.skip(reason='CUDA is unavailable')))
        else:
...
def _dtype_params(config: pytest.Config) -> list[object]:
    ...
    mapping = {'float32': torch.float32, 'float16': torch.float16, 'fp32': torch.float32, 'fp16': torch.float16}
