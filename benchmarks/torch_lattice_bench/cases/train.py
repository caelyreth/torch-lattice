...
def cases(
    ...
    *,
    ...
    specs = (
        ('conv3_implicit_unsorted_forward_backward', F.Dataflow.ImplicitGEMM, {'ifsort': False, 'split_mask_num': 1}),
        ('conv3_implicit_sorted_forward_backward', F.Dataflow.ImplicitGEMM, {'ifsort': True, 'split_mask_num': 3}),
        ('conv3_fetch_on_demand_forward_backward', F.Dataflow.FetchOnDemand, {'FOD_fusion': False}),
    )
    return tuple(
        BenchmarkCase(
            name=name,
...
def _setup_factory(device: torch.device, dataflow: F.Dataflow, kwargs: dict[str, Any]):
    def setup(params: dict[str, object]) -> TrainFixture:
        ...
        module = conv_module(base.channels, base.tensor.feats.dtype, device, 3, 1).train()
...
def _prepare(fixture: TrainFixture) -> TrainPrepared:
    return TrainPrepared(fresh_sparse(fixture.base.tensor), fixture.module, fixture.dataflow, fixture.kwargs)
...
def _backward(prepared: TrainPrepared) -> torch.Tensor:
    ...
    inp = SparseTensor(feats, prepared.x.coords, prepared.x.stride, prepared.x.spatial_range)
