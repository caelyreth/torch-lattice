...
def sparse_fixture(params: dict[str, object], *, device: torch.device) -> SparseFixture:
    ...
    feats = torch.randn(
        (points, channels), generator=generator, dtype=dtype, device="cpu"
    ).to(device)
...
def clone_sparse(x: SparseTensor, *, clone_feats: bool = True) -> SparseTensor:
    return x.replace(feats=x.feats.clone() if clone_feats else x.feats)
...
def sparse_coords(layout: str, n: int, device: torch.device) -> torch.Tensor:
    ...
    if layout == "line":
        i = torch.arange(n, device=device, dtype=torch.int64)
        return torch.stack(
            [torch.zeros_like(i), i, torch.zeros_like(i), torch.zeros_like(i)], dim=1
        ).int()
    if layout == "plane":
    ...
    base = torch.stack([bx.reshape(-1), by.reshape(-1), bz.reshape(-1)], dim=1)[
        :blocks_needed
    ]
    ...
    offs = torch.stack(
        [
            ox.reshape(-1),
            oy.reshape(-1),
            offsets.repeat_interleave(block * block)[: block**3],
        ],
        dim=1,
    )
    xyz = base.repeat_interleave(block**3, dim=0) * (block + 1) + offs.repeat(
        blocks_needed, 1
    )
