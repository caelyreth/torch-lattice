...
class Console:
    ...
    def start(self, case: Any, params: Mapping[str, Any], mode: str, device: str) -> None:
        if self.quiet:
            return
        self.current += 1
    ...
    def done(self, result: Any, *_: Any) -> None:
        if self.quiet:
            return
        if result.skipped:
            self.write(f'     {self.style("skip", "2")} {self.style(result.notes, "2")}')
            return
    ...
    def failed(self, case: Any, params: Mapping[str, Any], mode: str, device: str, error: BaseException) -> None:
        if self.quiet:
            return
        self.write(
    ...
    def skipped(self, case: Any, params: Mapping[str, Any], mode: str, device: str) -> None:
        if not self.quiet:
            self.write(
                f'     {self.style("skip", "2")} '
                f'{self.style(f"{device:<5}", "33")} '
    ...
    def _format_params(self, params: Mapping[str, Any]) -> str:
        preferred = ('N', 'points', 'channels', 'channels_in', 'channels_out', 'layout', 'dtype', 'kernel', 'stride')
        parts = [self._kv(key, params[key]) for key in preferred if key in params]
        parts.extend(self._kv(key, params[key]) for key in sorted(params) if key not in preferred)
        return ' '.join(parts)
    ...
    def _format_workload(self, workload: Mapping[str, Any]) -> str:
        ...
        for key, label in labels:
            ...
            if isinstance(value, int):
                ...
                parts.append(self._kv(label, f'{value:.2f}', key_style='2', value_style='36'))
        return ' '.join(parts)

    def _kv(self, key: str, value: Any, *, key_style: str = '2', value_style: str = '33') -> str:
        return f'{self.style(f"{key}=", key_style)}{self.style(str(value), value_style)}'
