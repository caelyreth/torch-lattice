...
at::Tensor hash_cuda(const at::Tensor idx) {
  ...
  TORCH_CHECK(idx.scalar_type() == at::kInt, "idx must use int32 dtype");
  TORCH_CHECK(idx.dim() == 2 && idx.size(1) == 4,
              "idx must have shape (N, 4)");
...
at::Tensor kernel_hash_cuda(const at::Tensor idx,
  ...
  TORCH_CHECK(idx.is_cuda() && kernel_offset.is_cuda(),
              "idx and kernel_offset must be CUDA tensors");
  TORCH_CHECK(idx.get_device() == kernel_offset.get_device(),
              "idx and kernel_offset must be on the same CUDA device");
  TORCH_CHECK(idx.scalar_type() == at::kInt &&
                  kernel_offset.scalar_type() == at::kInt,
              "idx and kernel_offset must use int32 dtype");
  TORCH_CHECK(idx.dim() == 2 && idx.size(1) == 4,
              "idx must have shape (N, 4)");
  TORCH_CHECK(kernel_offset.dim() == 2 && kernel_offset.size(1) == 3,
              "kernel_offset must have shape (K, 3)");
  TORCH_CHECK(idx.is_contiguous() && kernel_offset.is_contiguous(),
              "idx and kernel_offset must be contiguous");
  int N = idx.size(0);
