...
at::Tensor hash_cpu(const at::Tensor idx) {
  TORCH_CHECK(idx.device().is_cpu(), "idx must be a CPU tensor");
...
at::Tensor kernel_hash_cpu(const at::Tensor idx,
  ...
  TORCH_CHECK(idx.device().is_cpu() && kernel_offset.device().is_cpu(),
              "idx and kernel_offset must be CPU tensors");
