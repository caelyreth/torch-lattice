...
//                      0: fallback to torch_lattice 1.4 conv w/o optimizations
...
void group_strategy_generation(
  ...
  if (conv_mode == 2) {
    ...
    TORCH_CHECK(epsilon == 0, "epsilon must be zero outside conv_mode 2");
    TORCH_CHECK(mm_thresh == 0, "mm_thresh must be zero outside conv_mode 2");
    if (precompute_mid && kernel_volume % 2 != 0) {
      for (int i = 0; i < kernel_volume / 2; i++) {
        kernel_order.push_back(i);
      }
